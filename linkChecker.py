...
def send_mail(config, subject, message):
    ...
    """
    server = smtplib.SMTP(config['SMTPDomain'], 587)
    server.starttls()
...
def run():
    ...
    log_file_path = '{}/Broken-Link-Crawling-App/{}/'.format(gen_conf['LogFileDirectory'], gen_conf['SiteName'])
    ...
    if count_broken_links > 0:
        ...
        message = """
        Link Checker - FAILED, Broken links found
        Site: {}
        Date Scanned: {}
        No. Broken Links: {} \n
        ...
        message = """
        Link Checker - PASSED, No broken links found
        Site: {}
        Date Scanned: {}
